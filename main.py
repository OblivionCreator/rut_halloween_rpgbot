...
def getCandies(userid: int):
    ...
    with open(f"./candy_jar/{userid}.json", 'r') as c_file:
        ...
        if len(raw) == 2:
            candy, r_candy = raw
            return raw[candy], raw[r_candy], 0
        else:
            candy, r_candy, c_candy = raw
            return raw[candy], raw[r_candy], raw[c_candy]


def addCandies(userid, n_candy, n_r_candy, n_c_candy=False):
...
async def candyHandler(interaction):
    global hitpoints
    cur_can, r_c, temp = getCandies(interaction.author.id)
    candyToLose = 0
...
async def generate(ctx=None, channel=None):
    ...
    if victory:
        ...
        if tmpUSR != 0:
            vEmbed.add_field(name='Monster Defeated',
                             value=f'{bot.get_user(lastAttacker).name} got the final hit! They have been paid {reg_c} Kromer and {nar_c} Pipis! (+{min(50, nar_c / 2)}% Kromer Multiplier)\n{bot.get_user(tmpUSR).name} did the most damage, with {tmpDMG} damage! They have been paid {dmg_c} Kromer and {dmg_r} Pipis.',
                             inline=False)
        else:
            vEmbed.add_field(name='Monster Defeated',
                             value=f'{bot.get_user(lastAttacker).name} got the final hit! They have been paid {reg_c} Kromer and {nar_c} Pipis! (+{min(50, nar_c / 2)}% Kromer Multiplier)',
    ...
    for u in sorted_tCL:
        ...
        l_STR = f'{l_STR}{count}) {u_user.name}#{u_user.discriminator} - {sorted_tCL[u]} Kromer | {rareCandyLeaderboard[u]} Pipis ({min(50,(rareCandyLeaderboard[u]/2))}% Bonus Multiplier)\n'
        count += 1
...
async def attack(action, player_hp):
    ...
    if random.randrange(0, 100) >= rareChance:
        ...
        r_C = 0
    c_C, c_R, c_cc = getCandies(action.author.id)
